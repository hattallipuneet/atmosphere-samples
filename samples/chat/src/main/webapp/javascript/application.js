$(function () {
    ...
    request.onMessage = function (response) {
        ...
        if (!logged && myName) {
            ...
            var auto = json.author == 'autochat';
            var date = typeof(json.time) == 'string' ? parseInt(json.time) : json.time;
            addMessage(json.author, json.message, me ? 'blue' : auto ? 'green' : 'black', new Date(date));
        }
    };
    ...
    function addMessage(author, message, color, datetime) {
        if (author === 'autochat') {
            var newX = parseInt(message);
            if (newX !== 0 && newX !== (lastAuto + 1 )) {
                alert('lost messages! got ' + newX + ', expected ' + (lastAuto + 1));
                subSocket.close();
            } else {
                lastAuto = newX;
            }
            if(newX % 25 === 0) {
                content.html($('<p>', {text: 'Cleared'}));
            }
        }
